...
::: tip Tip:
This document is intended for readers who have received basic robotics training, have some basic development skills, and are familiar with JAKA's scripting language `JKS`.
...
Using AddOn can de-code and greatly simplify the process of setting instructions. It omits the complex settings in the programming step and encapsulates them into a simple, clear, and easy-to-understand instruction, which is easier to operate and more conducive to the reuse of programmed instructions, thus fully enhancing efficiency and creating results.
...
On the other hand, the AddOn of the Process Package class enables users to focus on the execution of specific processes rather than how the instructions are used, reducing the learning cost and difficulty of use for users.
...
AddOn custom services are developed based on node-red and run in the controller. When an AddOn is started, the controller starts the node-red process for that AddOn. The custom service can be used as an intermediate layer to provide services for custom commands and custom web pages. The relevant nodes provided in node-red can be used to query robot information, set robot parameters, and control robot motion.
...
When the App starts, it looks for the data for the custom command block from the AddOn package. If found, it is loaded into the Program-Extensions bar.

<div align="center"><img width="800"  src="./img/1.1-AboutAddOn/自定义指令查询.png"/></div>

When the custom command block is inserted into the application and you click Save or Run, the App makes a request to the AddOn service to send the parameters on the command block along with it.The AddOn service receives the request, splices the parameters into the script of the custom command, and returns it to the App.Finally, the App inserts the script it receives.

<div align="center"><img width="800"  src="./img/1.1-AboutAddOn/自定义指令保存.png"/></div>

* AddOn Custom Web Pages

Typically, custom web pages are deployed on a JAKA controller and are proxied through Nginx. Of course, it is also possible to deploy them on personal computers or other devices.

<div align="center"><img width="800"  src="./img/1.1-AboutAddOn/自定义网页访问.png"/></div>

Custom web pages can be used not only to access AddOn custom services or services from other devices, but also to interact with the JAKA App by using JavaScript methods injected into the web page by the JAKA App.
